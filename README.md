...
面向 [Claude Code](https://claude.ai/code) 的命理分析工具集，覆盖八字（四柱）、紫薇斗数、六爻（摇卦/纳甲）、奇门遁甲（时家）、姓名学（五格剖象）、测字（拆字）六种传统方法。通过交互式对话逐步收集出生信息或起卦条件，确定性的排盘/起卦计算由本地 Python 脚本完成（避免大语言模型凭记忆口算出错），解读环节则参照各方法对应的经典典籍与规则文档，并对存在流派分歧、置信度不足的环节如实标注，不作绝对化断言。
